...
* **极低资源占用**：15 -> 64 -> 48 -> 32 -> 3 的轻量级前馈神经网络。Flash 占用仅约 22.8KB，完美适配 STM32F103C8T6 等资源受限的 MCU。
* **远程索敌优化**：新增敌方速度分量(vel_along)和距离变化率(dist_rate)两个关键特征，使 AI 在远距离战斗中能预判敌方移动轨迹，大幅提升远程射击精度。
...
* **输入层 (15维)**: 包含双方相对距离、角度偏差、相对坐标差、双方血量、自身冷却状态、暴露系数、距墙距离、敌方沿视线方向速度分量(vel_along)、距离变化率(dist_rate)等。
* **隐藏层**: 
  - L1: 64神经元 (Tanh激活)
  - L2: 48神经元 (Tanh激活)
  - L3: 32神经元 (Tanh激活)
